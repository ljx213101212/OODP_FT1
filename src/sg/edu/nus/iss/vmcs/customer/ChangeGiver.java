...
public class ChangeGiver {
	...
	public boolean giveChange(int changeRequired) {
		...
		try {
			...
			for (int i = cashStoreSize - 1; i >= 0; i--) {
				StoreItem cashStoreItem = storeCtrl.getStore(Store.CASH)
						.getStoreItem(i);
				...
				txCtrl.getMainController().getMachineryController()
						.giveChange(i, quantityRequired);
			}
	...
	public boolean giveChangeCOR(int changeRequired){
		...
		//
		Currency currency = new Currency(changeBal);
		CoinDispenseChain coinChain = new CoinDispenseChain(storeCtrl,cashStoreSize);
		changeBal = coinChain.getC1().dispense(currency, txCtrl);	
		//
